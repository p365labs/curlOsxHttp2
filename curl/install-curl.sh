...
lipo \
	"/tmp/${CURL_VERSION}-${ARCH}/lib/libcurl.a" \
	-create -output lib/libcurl_Mac.a

echo "Update build dir"
if [ ! -e "${PWD}/../build/curl" ]; then
	echo "curl build not exists! creating..."
	echo "${PWD}/../build/curl"
	mkdir -p  ../build/curl
else 
	echo "Curl build dir already exists."
fi

cp /tmp/${CURL_VERSION}-${ARCH}/bin/curl ${PWD}/../build/curl/curl
